...
simulate_option_cost <- function(rates, ftp, maturity, cancellation, utilization, FTP_0){
  ...
  dftp_tau <- FTP_0 - (ftp[tau] + 0.0001 * ((T-tau) / 12 - 2)) 
  rates <- rates[tau:T]
...
simulate_client <- function(pooled_coefs, cancellation, maturity, volatility, type){
  T <- maturity * 12
  tau <- ceiling(cancellation * T)
  u <- rep(0,T)
  u[1] <- runif(1) 
...
#--- NPV_optioncost_alpha:  NPV of the option cost at quantile alpha  (!!DIFFERENT FOR DIFFERENT MATURITIES!!) 
#--- utilization:           Simulated Utilization Time Series         (From initialization until end)
simulate_eos <- function(cancellation, maturity, rates, NPV_optioncost_alpha, utilization){
  tau <- ceiling(cancellation * T)
  rates <- rates[1:tau]
  for(i in 1:length(rates)){
    rates[i] <- exp(-rates[i] * i/12)
  ...
  u <- utilization[1:tau]
  eos <- NPV_optioncost_alpha / sum(rates * utilization)
  return(eos)
}